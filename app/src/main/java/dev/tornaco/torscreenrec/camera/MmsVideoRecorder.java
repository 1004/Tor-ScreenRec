...
class MmsVideoRecorder extends MediaRecorder {
    ...
    public MmsVideoRecorder(final Camera camera, final int cameraIndex, final int orientation,
        ...
        setOutputFile(
                TorScreenRecApp.getApp().getApplicationContext().getContentResolver().openFileDescriptor(
    ...
    void cleanupTempFile() {
        ...
        SafeAsyncTask.executeOnThreadPool(new Runnable() {
            @Override
            public void run() {
                TorScreenRecApp.getApp().getApplicationContext().getContentResolver().delete(
                        tempUri, null, null);
            }
        });
        mTempVideoUri = null;
